...
int main(int argc, char* argv[]){
    ...
    if (argc < 3){
        cout<< "Usage: " <<argv[0]<< " <filename> <postname>"<<endl;
        return 1;
    }
    ...
    while (true){
        ...
        case 5:{
            ...
            string first,last;
            int amountPost;
            cin>>first>>last>>amountPost;

            string full = first + " " + last;
            int profId = network.getId(full);

            if(profId ==-1){
                cout<<full<<" was not found. Please Try again."<<endl;
            }
            else{
                string post = network.getPostsString(profId,amountPost);
                if(post.empty()){
