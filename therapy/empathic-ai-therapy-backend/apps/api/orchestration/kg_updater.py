...
from orchestration.insights import build_coach_insight, build_summary_partial
from orchestration.receipts import attach_receipts, validate_evidence_quote_present
from orchestration.safety import classify_safety_event
...
async def process_utterance_event(
    *,
    ...
    """
    Purpose:
        End-to-end pipeline for one finalized user EVI message: persist utterance/prosody, call Gemini, validate tool calls,
        apply Neo4j mutations, build receipts, and return websocket-ready diff payloads.

    Inputs:
        - `session_id`: active therapy session id
        - `evi_user_message`: dict matching `ws.protocol.EviUserMessagePayload`

    Returns:
        Result dict with expected keys:
        - `kg_diff`: `{nodes_upsert, edges_upsert, receipts}`
        - `tool_calls_applied`: `{calls, dropped_calls}`
        - optional `summary_partial`
        - optional `warnings`

    Data structures / implementation notes:
        Pipeline stages should be explicit and testable:
        1. Validate/drop interim messages
        2. Persist `Utterance` + `ProsodyFrame`
        3. Build graph context
        4. Build prompts + tool declarations
        5. Call Gemini and normalize tool calls
        6. Validate evidence and per-utterance caps
        7. Execute Neo4j repo mutations
        8. Build receipts + websocket payloads
    """
...
def build_graph_context_for_event(
    *,
    ...
    """
    Purpose:
        Retrieve compact graph context for the current utterance to improve entity/relation continuity.

    Inputs:
        - `session_id`: session identifier
        - `utterance_text`: finalized transcript text used for concept lookup hints

    Returns:
        Context dict suitable for `services.gemini.prompts.build_kg_user_prompt`.

    Data structures / implementation notes:
        - Delegate to `services.neo4j.repo_graph.get_graph_context_for_llm`
        - Bound size to protect latency and prompt token usage
    """
...
def apply_tool_calls_to_graph(
    *,
    ...
    """
    Purpose:
        Execute validated KG tool calls against Neo4j repositories and collect node/edge upserts.

    Inputs:
        - `session_id`: session id
        - `tool_calls`: normalized + validated Gemini tool calls
        - `utterance_record`: persisted utterance data (includes `utterance_id` / `message_id`)

    Returns:
        Dict with accumulated mutation results, e.g.:
        - `nodes_upsert: list[dict]`
        - `edges_upsert: list[dict]`
        - `tool_call_results: list[dict]`

    Data structures / implementation notes:
        - Implement deterministic tool dispatch by `name`
        - Maintain order for receipt transparency
        - Avoid duplicate upserts in a single event response when possible
    """
