...
async def fetch_access_token(
    *,
    ...
    """
    Purpose:
        Exchange Hume API key/secret for a temporary access token using the OAuth client-credentials endpoint.

    Inputs:
        - `api_key`: Hume API key (server-side secret)
        - `secret_key`: Hume secret key (server-side secret)
        - `http_client`: configured HTTP client (`httpx.AsyncClient` recommended)

    Returns:
        JSON-like dict with at least:
        - `access_token: str`
        - `expires_in: int`
        Optional:
        - `token_type`
    ...
    Data structures / implementation notes:
        - Request target: Hume `/oauth2-cc/token`
        - Authentication: Basic auth header from API key + secret
        - Handle non-2xx responses and map to internal `AppError`
    """
    if not api_key.strip():
        raise _build_app_error(
            code="hume_api_key_missing",
    ...
    if not secret_key.strip():
        raise _build_app_error(
            code="hume_secret_key_missing",
            message="Hume secret key is not configured.",
            http_status=500,
            retryable=False,
        )
...
def build_basic_auth_header(api_key: str, secret_key: str) -> str:
    """
    Purpose:
        Build the HTTP `Authorization: Basic ...` header value for Hume OAuth client credentials requests.
    ...
    Returns:
        Header value string in the format: `Basic <base64(api_key:secret_key)>`.

    Data structures / implementation notes:
        - Encode `api_key:secret_key` as UTF-8 bytes before Base64
        - Do not log the raw keys or the encoded token
    """
