...
class UpsertConceptNodeArgs(ToolArgsModel):
    """
    Purpose:
        Canonical arguments for the `upsert_concept_node` tool call emitted by Gemini.
    ...
    Expected fields to add:
        ...
        - `message_id: str`
        - `evidence_quote: str`
        - optional metadata: `confidence`, `aliases`, `attributes`

    Usage:
        Validated before executing Neo4j MERGE for session-scoped concept nodes.
    """
...
class UpsertRelationEdgeArgs(ToolArgsModel):
    """
    Purpose:
        Canonical arguments for the `upsert_relation_edge` tool call.

    Expected fields to add:
        - `from_label`, `from_canonical`
        - `rel_type`
        - `to_label`, `to_canonical`
        - `message_id`, `evidence_quote`

    Usage:
        Validated before Neo4j relationship MERGE between existing or to-be-created concept nodes.
    """
...
class LinkUtteranceMentionsArgs(ToolArgsModel):
    """
    Purpose:
        Arguments for linking a stored `Utterance` node to concept nodes via `MENTIONS` edges.

    Expected fields to add:
        - `utterance_id` or `message_id`
        - `mentions: list[dict]` with per-mention `label`, `canonical`, and optional offsets
        - `evidence_quote` and/or per-mention evidence spans

    Usage:
        Supports traceability from transcript to graph entities.
    """
...
class SetSessionGoalArgs(ToolArgsModel):
    """
    Purpose:
        Arguments for updating a session-level goal summary and linking it into the concept graph.

    Expected fields to add:
        - `canonical`
        - `message_id`
        - `evidence_quote`
        - optional `goal_status` or `priority`

    Usage:
        Can create/update a `Goal` node and optionally attach session metadata.
    """
...
class SkipKgUpdateArgs(ToolArgsModel):
    """
    Purpose:
        Arguments for the no-op tool used when forced function calling should result in no graph mutation.
    ...
    Usage:
        Prevents forced-tool mode from inventing KG mutations when the utterance is ambiguous or irrelevant.
    """
...
def normalize_tool_calls(raw_model_output: Any) -> list[dict[str, Any]]:
    """
    Purpose:
        Convert provider-specific Gemini function call output into a normalized list of tool call dictionaries.

    Inputs:
        - `raw_model_output`: SDK response represented as dict/object serialization.

    Returns:
        `list[dict]` where each item includes:
        - `name: str`
        - `arguments: dict[str, Any]`
        - optional metadata: `call_id`, `provider_index`

    Data structures / implementation notes:
        - Preserve call ordering as emitted by the model
        - Keep normalization provider-specific but output provider-agnostic
    """
...
def validate_tool_call_batch_limits(
    *,
    ...
    """
    Purpose:
        Enforce mutation volume limits on a single utterance to reduce hallucination blast radius.

    Inputs:
        - `tool_calls`: normalized tool calls proposed for one user message
        - `max_nodes_per_utterance`: cap for node upserts
        - `max_edges_per_utterance`: cap for edge upserts

    Returns:
        None. Raises validation error if batch exceeds limits.

    Data structures / implementation notes:
        - Count by tool name
        - Optionally return/record dropped calls in a future variant
    """
    node_count = 0
    edge_count = 0
